...
exports.insert = async function (body) {
    const {name,price,category,image,description,published,available,size} = body;
    console.log(body);
    const product = {
...
exports.update = async function (body) {
    const {_id,name,price,category,image,description,available,published,size} = body;
    const product = {
        _id: _id,
        name:name,
        price: Number(price),
